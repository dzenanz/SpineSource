...
{
	Vertex *v;
	//objWriteCell(qe, "D:\\hierarchyP.obj");
	//swapSubdivPos(qe);
	//objWriteCell(qe, "D:\\hierarchyS.obj");
	//swapSubdivPos(qe);

	for (int l=maxLevel-1; l>=0; l--) //update based on neighboring vertices only
	{
	...
	//objWriteCell(qe, "D:\\hierarchyN.obj");
	{
		CellVertexIterator cvi0(qe);
		while ((v = cvi0.next())!=0)
	...
	calculateSubdivisionPositions(qe, maxLevel, subdivFromLevel);
	{
		CellVertexIterator cvi0(qe);
		while ((v = cvi0.next())!=0)
			v->pos=v->subdivPos;

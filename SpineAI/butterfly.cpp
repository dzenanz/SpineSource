...
void normalizeHeuristic(Cell *qe, int maxLevel, int controlLevel)
{
	...
	for (int l=maxLevel-1; l>=0; l--) //normalize completely
	{
	...
	//objWriteCell(qe, "D:\\Temp\\hierarchyN.obj");
	{
		CellVertexIterator cvi0(qe);
		while ((v = cvi0.next())!=0)
			if (v->level<=controlLevel)
				v->subdivPos=v->pos;
	}
	calculateSubdivisionPositions(qe, maxLevel, controlLevel);
    ...
    unsigned &freeCount, double cifEdges) //, double cifSmoothness)
{
    ...
    Eigen::MatrixXd W(qe->countVertices()+qe->countEdges(), freeCount);
    W.setZero();
    
	CellVertexIterator cvi1(qe);
	while ((v = cvi1.next())!=0)
	{
		W.row(v->realIndex())=v->weights;
	}
    ...
    while ((v=cvi.next())!=0)
    {
        ...
        while ((e=vei.next())!=0)
        {
            //W.row(i+qe->countVertices())+=cifSmoothness*e->Dest()->weights;
            W.row(i+k+qe->countVertices())=cifEdges*(e->Dest()->weights+e->Oprev()->Dest()->weights);
            k++;
        }
        i+=v->valence; //i+=v->valence+1;
    }
